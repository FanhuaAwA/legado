...
| R-P2-003..011 | open                 | 番茄/短剧、缓存系统、Harmony 标注、`book` 对象绑定；2026-06-10 第二次复审新增：R-P2-008 前后端分离 WS 服务端（前端 useTransport 三模已就绪、后端缺 `/ws`）、R-P2-009 QuickJS Runtime 复用、R-P2-010 JS HTTP 桥线程池化；2026-06-10 用户要求前后端分离后新增：R-P2-011 前端绕过传输层修复（prefetch.ts 直连 Tauri listen、logger.ts 直连 invoke），架构纪律见 `docs/frontend-backend-separation.md` 与总纲第 60 节，详见审计文档第 3 节 |
