...
## 2026-06-15 PERF-MIAOGONGZI-LOCAL-IMPORT 状态更新

- 喵/猫公子订阅导入新增真实性能测试 `crates/reader-core/tests/miaogongzi_import_perf.rs`，默认 `#[ignore]`，手动访问 `http://yuedu.miaogongzi.net/shuyuan/miaogongziDY.json` 并在临时目录导入，不污染用户数据。
- 新增 `booksource_import_legacy_json_texts(items, smartExploreSubCategories, requestId?)`，多 URL 包/多本地文件共享同一核心批量文本导入链路；旧 `booksource_import_legacy_json_text` 保持兼容。
- 本地多文件导入改为并发读取文件后交给后端批量文本导入；前端 JSON parse/stringify 合并方案经测试为负优化，已移除。
- reader-core 导入批次写文件改为有上限并发，进度/写入批次从 25 调整为 100，SQLite `save_many` 改为分块多行 upsert。
- 实测喵/猫公子 10 包/1259 条：最终 `resolve_ms=1344`、URL 批量导入 `combined_ms=3831`；本地旧逐文件 `local_sequential_ms=4146`，新本地批量文本 `local_combined_ms=3807`，`local_speedup=1.09x`。
- 已同步 `docs/command-matrix.md`、`docs/reader-rust-route-b-spec.md`、`docs/source-compat-matrix.md`；本轮报告见 `reports/gates/2026-06-15-PERF-MIAOGONGZI-LOCAL-IMPORT/summary.md`。
