...
| NET-004   | closed (实现) / live 待验 | `http_doh_server` 已接入：新模块 `crawler/doh.rs` 实现 `reqwest::dns::Resolve`（JSON DoH API，**无新依赖**），bootstrap 客户端用 `.resolve(host, 已知IP)` 钉死避免递归，**fail-open** 到系统 DNS（任何 DoH 失败不破坏解析）。6 个 provider（alidns/dnspod/360dns/onedns/cloudflare/google）映射 JSON DoH 端点。主客户端经 `HttpClientConfig.doh_server` + `builder().dns_resolver(...)` 接入。单测 4（provider 映射、A/AAAA 解析、空响应、构建）+ http_client_config 增 1。⚠️ live DoH 解析未实测（离线环境，§6 not_run）：cloudflare/google/alidns/dnspod JSON DoH 标准；360dns/onedns 若 JSON 格式不符则 fail-open 到系统 DNS。JS 桥（blocking 客户端）仍走系统 DNS，未接 DoH |     |
...
| ID              | 任务                                                                                                                                                                                       | 验收                                                                                                   |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| NET-004-LIVE    | DoH live 验证：有网环境逐 provider（cloudflare/google/alidns/dnspod/360dns/onedns）对已知 host 发查询，确认真正走 DoH 而非 fail-open。`crawler/doh.rs` 可临时加日志或 `#[ignore]` 实网测试 | 各 provider 返回 Answer 且解析命中；不支持 JSON 的（疑 360dns/onedns）确认 fail-open 到系统 DNS 不报错 |
| NET-005         | 评估把 DoH 接入 JS 桥 blocking 客户端（`parser/js.rs:JS_HTTP_CLIENT`）。注意 blocking 客户端 + 异步 Resolve 的 runtime 交互需实测                                                          | JS 桥 java.ajax 也按 `http_doh_server` 走 DoH，或明确文档化只主客户端支持                              |
| SRC-FANQIE-LIVE | 番茄书源实网链路（blocked_by_js_api）：设备注册依赖 Packages.okhttp3 真实 OkHttp 行为，49 个 JS API 状态见 `docs/source-compat-matrix.md`                                                  | search→toc→content live_network_pass，或逐 API 明确降级                                                |
...
前后端接入审计已结清（NET-001~004 / CLEAN-001 已提交并推送，commit 82590e0~6b01446）。下一步按上文「后续维护任务路线图」取用，并先就「待用户决策」三项与用户确认取舍：

1. 有网/真机环境优先：A 段阻塞项（NET-004-LIVE DoH 实测、SRC-FANQIE-LIVE 番茄实网）。
2. 否则取 B 段某一能力域真实化（先确认用户是否需要该能力），按审计文档第 4 节处置规则实现，同步 `capabilities_get` + 前端入口 + `docs/command-matrix.md`。
3. 或推进 C 段 FORMB-ACCEPT 形态 B 浏览器闭环验收。

历史项 R-P2-003（番茄 JS API 缺口）并入 A 段 SRC-FANQIE-LIVE。
