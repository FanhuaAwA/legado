...
| onlyBackend                       |    0 | 无                                                                 |
| registered_implemented_count      |  109 | 全部已注册命令中的实现数量                                         |
| registered_unsupported_stub_count |   52 | 全部已注册命令中的 UNSUPPORTED stub                                |
| frontend_implemented_count        |  109 | 前端可触达且已实现                                                 |
| frontend_unsupported_stub_count   |   52 | 前端可触达但仅返回 UNSUPPORTED（2026-06-12 CAP-REPO 实现后 58→52） |
