...
pub async fn comic_get_page_sizes(
    ...
    Err(CommandError {
        ...
        message:
            "漫画缓存系统尚未实现，无法获取页面尺寸。请将漫画缓存模块标记为 unsupported_hidden。"
                .into(),
...
// ── 封面缓存 ──────────────────────────────────────────────
#[tauri::command]
pub async fn cover_cache_clear() -> CommandResult<()> {
    Err(unsupported("封面缓存清理"))
}
#[tauri::command]
pub async fn cover_cache_size() -> CommandResult<()> {
    Err(unsupported("封面缓存大小"))
}
#[tauri::command]
pub async fn cover_resolve_cache() -> CommandResult<()> {
    Err(unsupported("封面缓存解析"))
}
