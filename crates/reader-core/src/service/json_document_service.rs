...
impl JsonDocumentService {
    ...
    pub async fn list_namespaces(&self) -> Result<Vec<String>, AppError> {
        let rows = sqlx::query("SELECT DISTINCT namespace FROM json_documents ORDER BY namespace")
            .fetch_all(&self.pool)
            .await?;
        Ok(rows
            .iter()
            .map(|row| row.get::<String, _>("namespace"))
            .collect())
    }
