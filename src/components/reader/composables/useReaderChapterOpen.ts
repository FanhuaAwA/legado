...
interface UseReaderChapterOpenOptions {
  ...
  requestVipChapterPurchase?: (index: number, loadError: unknown) => Promise<boolean>;
...
export function useReaderChapterOpen(options: UseReaderChapterOpenOptions) {
  ...
  async function retryAfterVipPurchase(
    ...
    const purchased = await options.requestVipChapterPurchase(index, cause).catch(() => false);
  ...
  async function openPagedChapter(index: number, openOptions: OpenChapterOptions = {}) {
    ...
    try {
      ...
      await options.ensureParagraphCommentSummaries(index, text, openOptions.forceNetwork);
      if (token !== openChapterToken) {
        return;
      }
