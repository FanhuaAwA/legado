...
const sourceHomepageUrl = computed(() => normalizeExternalHttpUrl(props.src.homepageUrl));
</script>
...
<template>
  ...
  >
    ...
    <div class="src-card__actions">
      ...
      <n-button size="tiny" quaternary class="src-action" @click.stop="emit('export')"
        >导出</n-button
      >
      <n-button
        size="tiny"
...
.src-card--selected {
  border-color: var(--color-accent);
  border-left-color: var(--color-accent);
  background: var(--color-accent-subtle, rgba(var(--color-accent-rgb, 99 102 241) / 0.06));
}
